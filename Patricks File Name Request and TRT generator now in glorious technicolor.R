...
server <- function(input,output) {
  ...
  output$download_text <- downloadHandler(
    ...
    content = function(file) {
      ...
      ##to add another row in your .csv, simply add a comma and a carriage
      ##return after the last "'
      bork1 <- ('"borkborkbork","bork"') ##this will eventually be replaced with whatever
      ##traits you want to add in.  variable bork is one row, with columns 
      ##comma delineated
      bork2 <- ('"borkborkbork","bork"')
      writeLines(c(header, bork1, bork2), file) ##writes header line and all traits
      ##to .csv file
      
      ##here is where the rest of the .csv stuff will go.
    }
  )
}
