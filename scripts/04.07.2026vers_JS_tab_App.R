...
make_trait_reference <- function(data) {
  ...
  panels <- lapply(categories, function(cat_name) {
    category_data <- data %>% filter(category == cat_name)
    
    accordion_panel(
      title = cat_name,
      tagList(lapply(seq_len(nrow(category_data)), function(i) {
        div(style = "margin-bottom: 14px; padding-bottom: 10px; border-bottom: 1px solid #d9d9d9;",
            tags$strong(category_data$title[i]),tags$br(),tags$span(category_data$description[i]),
            tags$br(),
            tags$small(style = "color: #5f6b7a;",paste("Data type:", category_data$data_type[i])
            )
        )
      })
      )
  ...
  do.call(accordion, c(panels, list(open = FALSE)))
}
