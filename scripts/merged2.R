...
# read and clean maize traits
##SET YOUR OWN WORKING DIRECTORY
#setwd("~/Desktop/frontiers")
#maize_traits <- read_excel("maize_traits.xlsx") %>%
# rename(category = Catergory,
#       data_type = `data type`) %>%
#rrange(title)
...
# (hopefully) make sidebar panels
make_trait_reference <- function(data) {
  categories <- sort(unique(data$category))
  
  inner_panels <- lapply(categories, function(cat_name) {
    category_data <- data %>% filter(category == cat_name)
    ...
    accordion_panel(
      ...
      tagList(
        lapply(seq_len(nrow(category_data)), function(i) {
          div(
            style = "margin-bottom: 14px; padding-bottom: 10px; border-bottom: 1px solid #d9d9d9;",
            tags$strong(category_data$title[i]),
            tags$br(),
            tags$span(category_data$description[i]),
            tags$br(),
            tags$small(
              style = "color: #5f6b7a;",
              paste("Data type:", category_data$data_type[i])
            )
          )
        })
      )
  ...
  accordion(
    accordion_panel(
      title = "Trait Descriptors",
      do.call(accordion, c(inner_panels, list(open = FALSE)))
    ),
    open = FALSE
  )
}
...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    fg = "#1a1a3e",
    primary = "#ffb6c1",
    make_trait_reference(maize_traits)
  ),
  card(
    style = "background-color: #ffffc5; 
    border-color: #ffb6c1;
    border-width: 3px;
    height: 50%",
    card_header(
      style = "color: #1a1a2e; font-weight: bold; font-size: 20px;",
      "Choices"
    ),
    pickerInput(
      inputId = "checkGroup",
